...
export const EditorAlbumTitles = ({ dispatch, groups, highlighted }) => {
    const elements = groups.map((group, index) => {
        const titles = group.map(({ id, name }) => _jsxs(AlbumTitle, { dispatch: dispatch, id: id, name: name, highlighted: highlighted, children: [_jsx(RenameAlbumButton, { dispatch: dispatch, id: id }, void 0), _jsx(DeleteAlbumButton, { dispatch: dispatch, id: id }, void 0)] }, id));
        return (_jsx(AlbumTitleGroup, { children: titles }, index));
    });
    return (_jsx(AlbumTitlesContainer, { dispatch: dispatch, children: elements }, void 0));
};
